...
# ============================================================
# WEZApproachAction — Sprint to WEZ range when already aligned
# ============================================================

class WEZApproachAction(BaseAction):
    """Sprint to WEZ range while maintaining heading when ATA is already low.

    Use when ATA < 20° but distance is outside 3000 ft WEZ max range.
    Combines full afterburner to close range with PD heading to hold ATA low.
    GunEngagement [2] / SnapShot [3] fire automatically once inside WEZ range.

    Speed profile:
      dist > 3000 ft : full afterburner — close to WEZ as fast as possible
      dist 1500–3000 : hold speed — already in WEZ range, focus on heading
      dist < 1500 ft : decel — prevent overshoot below 500 ft WEZ minimum
    """

    _KP = 0.9
    _KD = 0.3

    def __init__(self, name: str = "WEZApproachAction") -> None:
        super().__init__(name)
    ...
    def update(self) -> py_trees.common.Status:
        try:
            ...
            # PD heading — hold ATA low while closing.
            # Guard against stale prev_tau after branch inactivity: any jump
            # > 20° in one 0.1s tick is physically impossible, so treat as first tick.
            if (
                self._prev_tau is not None
                and abs(tau_deg - self._prev_tau) < 20.0
            ):
                tau_rate = (tau_deg - self._prev_tau) / 0.1
                cmd = self._KP * tau_deg + self._KD * tau_rate
            else:
                cmd = self._KP * tau_deg
            self._prev_tau = tau_deg
            heading_idx = max(0, min(8, int(round(cmd / 22.5)) + 4))

            # Speed: sprint from outside WEZ, stabilize inside
            if distance_ft > 3000:
                vel_idx = 4   # full afterburner — need to reach WEZ range
            elif distance_ft > 1500:
                vel_idx = 2   # in WEZ range — hold, gun branches handle it
            else:
                vel_idx = 1   # very close — decel to avoid passing below 500 ft min

            self.set_action(2, heading_idx, vel_idx)   # hold altitude
            return py_trees.common.Status.SUCCESS

        except Exception as exc:
            logger.warning(f"WEZApproachAction error: {exc}")
